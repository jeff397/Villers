...
export default function SalleDesFetes() {
  const capacite = dataSalle?.capacite || {
    assise: 0,
    debout: 0,
    description: "",
  };
  const tarifs = dataSalle?.tarifs || {
    habitantsCommune: { weekEnd: "—", journee: "—" },
    exterieurs: { weekEnd: "—", journee: "—" },
    options: { vaisselle: "—", chauffageElectricite: "" },
  };
  const conditions = dataSalle?.conditions || {
    caution: "—",
    acompte: "—",
    documentsRequis: [],
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 bg-slate-50 p-6 rounded-2xl border border-slate-200/60">
        <div className="md:col-span-2 space-y-4">
          ...
          {conditions.documentsRequis &&
          conditions.documentsRequis.length > 0 ? (
            ...
            <p className="text-sm text-slate-500 italic">
              Contactez la mairie pour connaître les documents requis.
            </p>
          )}
        </div>
        ...
        <div className="space-y-3 border-t md:border-t-0 md:border-l border-slate-200 pt-4 md:pt-0 md:pl-6 text-sm text-slate-600">
          ...
          <p>
            <span className="font-semibold text-slate-700">Caution :</span>{" "}
            {conditions.caution}
          </p>
          <p>
            <span className="font-semibold text-slate-700">Arrhes :</span>{" "}
            {conditions.acompte}
          </p>
          {tarifs.options?.vaisselle && (
            <p>
          ...
          {tarifs.options?.chauffageElectricite && (
            <p className="text-xs text-slate-500 italic mt-2 border-t border-slate-200/60 pt-2">
              {tarifs.options.chauffageElectricite}
            </p>
          )}
        </div>
      ...
      <div className="mt-10 text-center">
        ...
        <a
          href="#contact"
          className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all shadow-sm shadow-purple-900/20 cursor-pointer"
        >
