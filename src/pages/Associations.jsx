...
import listeAssociationsData from "../data/associations.json";

function Associations() {
  // 1. Extraction sécurisée du tableau d'associations
  const listeAssociations = listeAssociationsData.associations || [];
  ...
  const categories = [
    "Tous",
    ...new Set(listeAssociations.map((a) => a.categorie).filter(Boolean)),
  ];

  // Filtrer la liste
  const associationsFiltrées = listeAssociations.filter((asso) => {
    const nomAsso = asso.nom || "";
    const descAsso = asso.description || "";
    const catAsso = asso.categorie || "";

    const correspondCategorie =
      categorieFiltre === "Tous" || catAsso === categorieFiltre;
    const correspondRecherche =
      nomAsso.toLowerCase().includes(recherche.toLowerCase()) ||
      descAsso.toLowerCase().includes(recherche.toLowerCase());
      ...
      {associationsFiltrées.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {associationsFiltrées.map((asso, index) => (
            <div
              key={index}
              onClick={() => setAssoSelectionnee(asso)}
              ...
              <div className="border-t border-slate-100 pt-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {assoSelectionnee.telephone && (
                    ...
                    >
                      <Phone className="w-5 h-5 text-slate-400 group-hover:text-emerald-600 animate-none" />
                      <div>
